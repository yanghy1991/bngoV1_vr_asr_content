...
static int abnormal_weather_flag = 0;
...
int szhy_read_msgEsp(int sock,unsigned char *buffer,int buflen)
{
	...
	if(rc < 0){
		printf("[%s] [%d] get szhy_read_msgEsp failed\n",__func__,__LINE__);
		return -1;
	}
...
int get_szhyServerEsp(char *buffer)
{
	...
	int ret = 0;
	DEBUG("\n p:%s\n",resp->valuestring);
	if(abnormal_weather_flag){
		ret = get_weatherEsp(resp->valuestring);
	} else {
		ret = msq_send(MOZART_QUEUE,ASR_FLAG,resp->valuestring);
	}
...
{
	abnormal_weather_flag = 1;
	send_szhyServerReq(sock,cmd);
...
int get_weatherEsp(char *buffer)
{
	...
	char weatherData[1024] = {0};
	strcpy(weatherData,buffer);

	char *json_str = NULL;
	json_str = strstr(weatherData,"{");
	...
	//获取sds
	cJSON *sds = NULL;
	sds = cJSON_GetObjectItem(results, "sds");
	if(sds == NULL)
	{
		printf("%s : get sds failed!\n",__func__);
		goto error_exit;
		//获取err_no失败
	}
	...
	//获取output
	cJSON *output = NULL;
	output = cJSON_GetObjectItem(sds, "output");
	if(output == NULL)
	{
		printf("%s : get sds failed!\n",__func__);
		goto error_exit;
	...
	int ret = msq_send(MOZART_QUEUE,WEATHER_FLAG,output->valuestring);

	cJSON_Delete(root);
	abnormal_weather_flag = 0;
	return ret;
